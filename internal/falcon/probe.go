...
// request against the Falcon API. Connectivity is reported as connected when
// the token endpoint returns 201 Created.
//
...
func probeConnectivity(ctx context.Context, cfg *config.Config, httpClient *http.Client) bool {
	...
	// Falcon's OAuth2 token endpoint returns 201 Created on success.
