Falcon Query Language (FQL) - Search Exclusions Guide

Use this guide to build the `filter` parameter for `falcon_search_exclusions`.
The supported fields depend on the `exclusion_type` you are searching (ioa, ml,
sensor_visibility, certificate). Pick the matching section below.
...
• > = greater than: created_on:>'now-7d' (timestamp fields)
• >= = greater than or equal: created_on:>='now-7d'
• < = less than: created_on:<'now-24h'
• <= = less than or equal: created_on:<='now-24h'
• :* = wildcard match operator: value:*'*substring*' (see filtering caveats)

=== DATA TYPES & SYNTAX ===
• String: 'value'
• Boolean: true or false (no quotes)
• Timestamp: 'YYYY-MM-DDTHH:MM:SSZ' or relative ('now-7d', 'now-24h'; lowercase, quoted)

=== COMBINING CONDITIONS ===
• + = AND condition: applied_globally:true+created_on:>'now-7d'
• , = OR condition: status:'enabled',status:'disabled'
• ( ) = Group expressions
...
1. Unsupported filter fields return an empty result, not an error. These query
APIs do not validate filter fields — filtering on a field that is not listed
in the matching section's table silently returns zero matches instead of a
400. An empty result therefore does NOT prove nothing exists; if a search
comes back empty, re-check every field against the table below before
concluding there are no matches.
2. `value`/`name` matching depends on the operator. The plain `:` operator is an
exact, full-string match — `value:'/usr/local'` matches only an entry whose
...
containing `Signer`. The `~` (tilde/contains) operator is NOT supported on
these fields and silently returns nothing.
