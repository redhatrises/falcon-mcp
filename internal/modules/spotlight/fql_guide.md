...
Use this guide to build the `filter` parameter for `falcon_search_vulnerabilities`.
This module queries CrowdStrike Falcon Spotlight vulnerabilities.
...
• * = wildcard matching (not supported on all fields — see notes below)
...
• + = AND condition (e.g., status:'open'+cve.severity:'HIGH')
• , = OR condition (e.g., status:'open',status:'reopen')
• ( ) = Group expressions
