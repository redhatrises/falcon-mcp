...
// FQL_FILTER_HINT_SUFFIX (falcon_mcp/common/fql.py).
const fqlFilterHintSuffix = "FQL uses + for AND and , for OR (not the words AND/OR). " +
	"Values must be single-quoted."

// filterHints maps a "falcon_"-prefixed tool name to its curated inline FQL
// field hint. It mirrors upstream falcon-mcp's FILTER_HINTS dict.
var filterHints = map[string]string{
	// === Detections ===
	"falcon_search_detections": "Common fields: severity_name (Critical|High|Medium|Low|Informational), " +
	...
	"falcon_count_kubernetes_containers": "Common fields: cluster_name, namespace, container_name, " +
		"image_repository, running_status (true|false), cloud_name, cloud_region.",

	// === Cloud: Image Vulnerabilities ===
	"falcon_search_images_vulnerabilities": "Common fields: cve_id, severity (Critical|High|Medium|Low|Unknown), " +
		"cvss_score, registry, repository, tag, container_running_status (running|stopped).",

	// === Cloud: CSPM Assets ===
	"falcon_search_cspm_assets": "Common fields: cloud_provider (aws|azure|gcp), account_name, " +
	...
	"falcon_search_cloud_risks": "Common fields: severity (Critical|High|Medium|Low|Informational), " +
	...
	// === Correlation Rules ===
	"falcon_search_correlation_rules": "Common fields: name, status (active|inactive), state (published|unpublished|draft), " +
	...
	"falcon_search_ioa_rule_groups": "Common fields: platform (windows|mac|linux), name, enabled (true|false), " +
		"rules.pattern_severity (critical|high|medium|low|informational), " +
		"rules.ruletype_name, created_on (UTC datetime).",

	// === Discover: Applications ===
	"falcon_search_applications": "Common fields: name, vendor, category, is_suspicious (true|false), " +
	...
	"falcon_search_unmanaged_assets": "Common fields: hostname, platform_name (Windows|Linux|Mac), " +
	...
	// === Intel: Actors ===
	"falcon_search_actors": "Common fields: name, actor_type, known_as, " +
	...
	// === Intel: Indicators ===
	"falcon_search_indicators": "Common fields: type (hash_md5|hash_sha256|domain|ip_address|url|email_address), " +
	...
	// === Intel: Reports ===
	"falcon_search_reports": "Common fields: name, type, sub_type, actors, " +
		"target_countries, target_industries, tags, " +
		"created_date (UTC datetime), last_modified_date (UTC datetime).",

	// === IOC ===
	"falcon_search_iocs": "Common fields: type (domain|ipv4|ipv6|md5|sha256), " +
		"action (detect|prevent|allow), severity_number (1-5), " +
		"source, applied_globally (true|false), expired (true|false), " +
		"created_on (UTC datetime).",

	// === RTR Sessions ===
	"falcon_search_rtr_sessions": "Common fields: hostname, user_id, origin, " +
		"created_at (UTC datetime), offline_queued (true|false), " +
		"base_command.",

	// === Quarantine ===
	"falcon_search_quarantined_files": "Common fields: hostname, sha256, state (quarantined|released), " +
		"date_updated (UTC datetime), paths.",
	"falcon_preview_quarantine_actions": "Common fields: hostname, sha256, state (quarantined|released), " +
		"date_updated (UTC datetime), paths.",
	"falcon_update_quarantined_files": "Common fields: hostname, sha256, state (quarantined|released), " +
		"date_updated (UTC datetime), paths.",
	"falcon_delete_quarantined_files": "Common fields: hostname, sha256, state (quarantined|released), " +
		"date_updated (UTC datetime), paths.",

	// === Exclusions ===
	"falcon_search_exclusions": "Fields vary by exclusion_type. Common: applied_globally (true|false), " +
	...
	// === Host Groups ===
	"falcon_search_host_groups": "Common fields: name, group_type (static|dynamic|staticByID), " +
		"created_by, created_timestamp (UTC datetime), " +
		"modified_by, modified_timestamp (UTC datetime).",
	"falcon_search_host_group_members": "Filters on HOST (device) attributes: hostname, platform_name (Windows|Linux|Mac), " +
		"status (normal|contained), local_ip, external_ip, os_version, last_seen, " +
		"product_type_desc (Workstation|Server|Domain Controller).",
	"falcon_perform_host_group_action": "Filters on HOST (device) attributes to select members for the action: " +
	...
	// === Policies ===
	"falcon_search_policies": "Common fields: platform_name (Windows|Linux|Mac; 'all' for content_update), " +
	...
	// === Data Protection ===
	"falcon_search_data_protection_classifications": "Common fields: name, created_by, created_at (UTC datetime), " +
		"modified_by, modified_at (UTC datetime).",
	"falcon_search_data_protection_policies": "Common fields: name, description, is_enabled (true|false), " +
		"is_default (true|false), precedence, created_at (UTC datetime), modified_by.",
	"falcon_search_data_protection_content_patterns": "Common fields: name, category, type, region, " +
		"example, deleted (true|false).",

	// === Recon ===
	"falcon_search_recon_notifications": "Common fields: status (new|in-progress|closed-false-positive|closed-true-positive), " +
	...
	"falcon_search_recon_rules": "Common fields: status (active), " +
	...
	"falcon_search_recon_exposed_data_records": "Common fields: domain, email, " +
	...
	"falcon_search_report_executions": "Common fields: scheduled_report_id, status (Success|Failed|Pending|Running), " +
		"type, created_on (UTC datetime).",

	// === Sensor Usage ===
	"falcon_search_sensor_usage": "Common fields: event_date (YYYY-MM-DD format, e.g. event_date:'2024-06-11'), " +
		"period (number of days as quoted string, e.g. period:'30'; min 1, max 395, default 28).",

	// === Serverless Vulnerabilities ===
	"falcon_search_serverless_vulnerabilities": "Common fields: cve_id, severity (Critical|High|Medium|Low|Unknown), " +
		"cloud_provider (aws|azure|gcp), function_name, " +
		"application_name, runtime, cvss_base_score.",

	// === Spotlight Vulnerabilities ===
	"falcon_search_vulnerabilities": "Common fields: cve.id, cve.severity (Critical|High|Medium|Low), " +
