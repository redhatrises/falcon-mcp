...
// mutate shared client token state (mirrors Python _login_handler(stateful=False)).
...
// coreTools owns the three Python-parity core tools. list_enabled_modules is
...
// which Python exposes only outside dynamic mode.
...
// The shape matches the pre-existing Go dynamic-mode envelope (name +
// description + total), which is a superset of Python's name-only list.
